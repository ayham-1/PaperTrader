...
\begin{center}
	\begin{tikzpicture}[>=stealth,every node/.style={shape=rectangle,draw,rounded
		...
		\node (mastercom) [above right=of workerserver] {Master Server Communication};
		\node (cachemanage) [above=of workerserver] {Cache Management};
		\node (clientmanage) [above left=of workerserver] {Client Management};
		\node (accauth) [below left=of workerserver] {Account Authorization};
		...
		\draw[<->, dashed] (masterserver.west) to [bend right=15] (clients.west);
...
The account management module  should be able to do the following things:
\begin{itemize}
...
into the database (using the database management \ref{Database Management}).
...
with a password, and the user should be asked to sign in with the third-party
credentials. It should also be possible to add a password to the account marked
to be 'logginable' with third-party logins, making it possible to login with
the password and using third-party logins.
